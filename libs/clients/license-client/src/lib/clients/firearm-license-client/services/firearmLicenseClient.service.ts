...
export class FirearmLicenseClient implements LicenseClient<FirearmLicenseDto> {
  ...
  private async fetchLicenseData(
    ...
    //i hate this
    if (responses.licenseInfo) {
      data.licenseInfo = responses.licenseInfo
    } else {
      return {
        ok: false,
        error: {
          code: 11,
          message: 'Firearm license fetch failed',
        },
      }
    }

    if (responses.categories) {
      data.categories = responses?.categories
    } else {
      return {
        ok: false,
        error: {
          code: 11,
          message: 'Firearm license categories failed',
        },
      }
    }
